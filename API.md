...
### `GET https://api-videohub.naver.com/shortformhub/feeds/v9/card` ⭐
**클립 재생 정보 (play-info)** — 2026-04-26 HAR 캡처에서 발견. CloudStream `loadLinks` (클립)에서 사용.

**호스트가 다릅니다**: `api.chzzk.naver.com`이 아닌 **`api-videohub.naver.com`**.

**Query**
| 이름 | 예시 | 설명 |
|---|---|---|
| `seedType` | `SPECIFIC` | 단일 클립 조회 |
| `serviceType` | `CHZZK` | |
| `seedMediaId` | `067592FC9E0AE2445DD918FE3958D07A28F3` | clipDetail의 `videoId` (36-char hex) |
| `mediaType` | `VOD` | |
| `panelType` | `sdk_chzzk` | |
| `referer` | `https://chzzk.naver.com/clips/{clipUID}` | |
| `recType` | `CHZZK` | |
| `recId` | `{"seedClipUID":"...","fromType":"GLOBAL","listType":"RECOMMEND"}` | clipDetail의 `recId` (없으면 직접 구성) |
| `enableReverse` | `false` | |
| `adAllowed` | `Y` | |
| `clickNsc` | `chzzk_url_clip` | (telemetry, 임의 값 OK) |
| `clickArea` | `clip_item` | (telemetry, 임의 값 OK) |
| `deviceType` | `html5_mo` | `pc`, `sdk_chzzk` 등도 가능 |

**Response (DASH/MPD 형태)**
```jsonc
{
  "session": { "id": "...", "expireAt": 1777162957948, "hasMore": true },
  "card": {
    "index": 1,
    "content": {
      "serviceType": "CHZZK",
      ...
      "vod": {
        ...
        "playback": {
          "$version": "3.0.0",
          "MPD": [{
            "@mediaPresentationDuration": "PT10.034S",
            "Period": [{
              "@duration": "PT10.034S",
              "AdaptationSet": [
                {
                  ...
                  "Representation": [{
                    ...
                    "@frameRate": "30",
                    "BaseURL": ["https://b02-kr-smp-vod.pstatic.net/glive-clip/c/read/v2/VOD_ALPHA/glive-clip/<videoId>/pd/<ts>/<uuid>.mp4?hdnts=exp=...~acl=...~hmac=..."]
                  }]
                },
                {
                  "@mimeType": "video/mp2t",
                  "Representation": [{
                    "@id": "<uuid>",
                    "BaseURL": ["https://b02-kr-smp-vod.pstatic.net/glive-clip/c/read/v2/VOD_ALPHA/glive-clip_<date>_<seq>/hls/"]
                  }]
                }
              ]
            }]
...
**핵심 필드**
- `card.content.vod.playable` — false면 재생 불가 (만료/제거됨)
- `card.content.vod.playback.MPD[0].Period[0].AdaptationSet[]` — 두 가지 adaptation
  - `mimeType: "video/mp4"` → **progressive MP4 직접 다운로드** (단일 화질, manifest 불필요) ← 권장
  - `mimeType: "video/mp2t"` → HLS TS 세그먼트 디렉토리 (m3u8 별도 구성 필요)
- `BaseURL[0]` — 첫 번째가 실제 재생 URL

**클립 CDN 호스트**: `b02-kr-smp-vod.pstatic.net` (live의 `nvelop-livecloud.pstatic.net`와 다름)

**재생 URL 구조**
- MP4: `https://b02-kr-smp-vod.pstatic.net/glive-clip/c/read/v2/VOD_ALPHA/glive-clip/{videoId}/pd/{epoch_ms}/{uuid}.mp4?hdnts=...`
- HLS 세그먼트: `https://b02-kr-smp-vod.pstatic.net/glive-clip/c/read/v2/VOD_ALPHA/glive-clip_{YYYY_MM_DD}_{seq}/hls/{uuid}-{NNNNNN}.ts?hdnts=...`
- 인증 토큰 `hdnts=exp={unix}~acl=...~hmac=...` — 수 시간 만료, 매 호출마다 fresh URL 받기

### 권장 호출 순서 (클립)
1. `GET /service/v1/clips/{clipUID}/detail?optionalProperties=...` 으로 메타데이터 + `videoId` 확보
2. `GET https://api-videohub.naver.com/shortformhub/feeds/v9/card?seedMediaId={videoId}&seedType=SPECIFIC&...` 으로 play-info 조회
3. `card.content.vod.playback.MPD[0].Period[0].AdaptationSet[]`에서 `mimeType=="video/mp4"` Representation 선택
4. `BaseURL[0]`을 ExtractorLink로 emit (단일 화질 MP4, M3u8Helper 불필요)
...
| `loadLinks` (클립) | `GET https://api-videohub.naver.com/shortformhub/feeds/v9/card?seedMediaId={videoId}&...` → `card.content.vod.playback.MPD[].Period[].AdaptationSet[].Representation[].BaseURL[0]` (mp4 직접 재생) |
...
### 권장 호출 순서 (클립) — §클립 섹션과 동일, 검증 완료
1. `GET /service/v1/clips/{clipUID}/detail?optionalProperties=...` 으로 메타데이터 + `videoId` 확보
2. `GET https://api-videohub.naver.com/shortformhub/feeds/v9/card?seedMediaId={videoId}&seedType=SPECIFIC&serviceType=CHZZK&...` 으로 play-info 조회
3. `card.content.vod.playback.MPD[0].Period[0].AdaptationSet[]`에서 `mimeType=="video/mp4"` Representation 선택
4. `BaseURL[0]`을 ExtractorLink로 emit (단일 화질 MP4, M3u8Helper 불필요)
